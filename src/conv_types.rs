...
pub trait ConvFunction<T> : ConvInfo
    where T: RealNumber {
    fn calc(&self, x: T) -> T;
}
...
pub struct LinearTableLookup<T>
    where T: RealNumber {
    table: Vec<T>,
    domain: ConvDomain,
    delta: T,
...
macro_rules! add_linear_table_lookup_impl {
    ($($data_type:ident),*) => {
        $(
            impl ConvInfo for LinearTableLookup<$data_type> {               
                fn domain(&self) -> ConvDomain {
                    self.domain
                }
            }

            impl ConvFunction<$data_type> for LinearTableLookup<$data_type> {                
                fn calc(&self, x: $data_type) -> $data_type {
            ...
            impl LinearTableLookup<$data_type> {               
                pub fn from_raw_parts(table: &[$data_type], domain: ConvDomain, delta: $data_type, offset: $data_type) -> Self {
                    let owned_table = Vec::from(table);
                    LinearTableLookup { table: owned_table, domain: domain, delta: delta, offset: offset }
                }
                
                pub fn from_conv_function(other: &ConvFunction<$data_type>, delta: $data_type, offset: $data_type, len: usize) -> Self {
                    ...
                    LinearTableLookup { table: table, domain: other.domain(), delta: delta, offset: offset }
                }
            }
        )*
    }
}
add_linear_table_lookup_impl!(f32, f64);
...
impl<T> ConvFunction<T> for RaiseCosineFuncton<T> 
    where T: RealNumber {
    fn calc(&self, x: T) -> T {
        if x == T::zero() {
            return T::one();
...
mod tests {
    ...
    fn lookup_table32_test() {
        let window = RaiseCosineFuncton::new(0.35);
        let table = LinearTableLookup::<f64>::from_conv_function(&window, 0.2, 0.0, 10);
        let expected = 
            [0.0, 0.2171850639713355, 0.4840621929215732, 0.7430526238101408, 0.9312114164253432, 
             1.0, 0.9312114164253432, 0.7430526238101408, 0.4840621929215732, 0.2171850639713355];
    ...
    fn linear_interpolation_lookup_table32_test() {
        let window = RaiseCosineFuncton::new(0.35);
        let table = LinearTableLookup::<f64>::from_conv_function(&window, 0.4, 0.0, 10);
        let expected = 
            [0.0, 0.2171850639713355, 0.4840621929215732, 0.7430526238101408, 0.9312114164253432, 
             1.0, 0.9312114164253432, 0.7430526238101408, 0.4840621929215732, 0.2171850639713355];
