    ...
    where T: RealNumber
{
    ...
    /// assert_eq!([3.0, 2.0, 5.0, 4.0], complex.interleaved(0..));
    /// ```
    ...
    /// let complex = ops.get().expect("Ignoring error handling in examples");
    /// assert_eq!([1.0, 2.0, 3.0, 4.0], complex.interleaved(0..));
    /// ```
    ...
    /// let complex = ops.get().expect("Ignoring error handling in examples");
    /// assert_eq!([2.0, 4.0, 6.0, 8.0], complex.interleaved(0..));
          ...
          N: NumberSpace
{
    ...
    number_space: PhantomData<N>
}

impl<T, D, N> Identifier<T> for Ident<T, D, N>
    where T: RealNumber,
          D: Domain,
          N: NumberSpace {
    type Vector = DspVec<S, T, D, N>;
