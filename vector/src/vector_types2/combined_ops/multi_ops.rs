...
use std::result;
use super::super::{
	ToSlice, ToSliceMut, Owner, NumberSpace, Domain,
	RealOrComplexData, TimeOrFrequencyData,
	DspVec, ErrorReason, RededicateForceOps, Buffer
};
...
pub struct MultiOperation2<S, T, NO1, DO1, NO2, DO2>
    ...
    prepared_ops: PreparedOperation2<T,
					RealOrComplexData, TimeOrFrequencyData, RealOrComplexData, TimeOrFrequencyData,
					NO1, DO1, NO2, DO2>
}


/// Creates a new multi operation for one vectors.
pub fn multi_ops1<S, T, NI, DI>(vector: DspVec<S, T, NI, DI>)
    -> MultiOperation1<S, T, NI, DI>
...
pub fn multi_ops2<S, T, NI1, DI1, NI2, DI2>(a: DspVec<S, T, NI1, DI1>, b: DspVec<S, T, NI2, DI2>)
    -> MultiOperation2<S, T, NI1, DI1, NI2, DI2>
	where S: ToSliceMut<T> + Owner,
		  DspVec<S, T, NI1, DI1>: RededicateForceOps<DspVec<S, T, RealOrComplexData, TimeOrFrequencyData>>,
		  DspVec<S, T, NI2, DI2>: RededicateForceOps<DspVec<S, T, RealOrComplexData, TimeOrFrequencyData>>,
	   	  T: RealNumber + 'static,
...
impl<S, T, NO, DO> MultiOperation1<S, T, NO, DO>
	where S: ToSliceMut<T> + Owner,
	  ...
	  pub fn extend<NI2, DI2>(self, vector: DspVec<S, T, NI2, DI2>)
		...
		let extend = PreparedOperation2
		{
	  	...
	  	MultiOperation2 { a: a, b: b, prepared_ops: extend }
	}

	/// Executes all recorded operations on the stored vector.
	pub fn get<B>(self, buffer: &mut B) -> result::Result<DspVec<S, T, NO, DO>, (ErrorReason, DspVec<S, T, NO, DO>)>
		where B: Buffer<S, T> {
...
impl<S, T, NO1, DO1, NO2, DO2> MultiOperation2<S, T, NO1, DO1, NO2, DO2>
	...
	/// Executes all recorded operations on the stored vector.
	pub fn get<B>(self, buffer: &mut B)
	...
	/// placeholder for vectors. Every operation done to an `Identifier`
	/// is recorded and will be executed on vectors if `get` is called.
	pub fn add_ops<F, NT1, DT1, NT2, DT2>(self, operation: F)
        -> MultiOperation2<S, T, NT1, DT1, NT2, DT2>
        where F: Fn(Identifier<T, NO1, DO1>, Identifier<T, NO2, DO2>) -> (Identifier<T, NT1, DT1>, Identifier<T, NT2, DT2>),
              DT1: Domain, NT1: NumberSpace,
              DT2: Domain, NT2: NumberSpace
	{
		let ops = self.prepared_ops.add_ops(operation);
